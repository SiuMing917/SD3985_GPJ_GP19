...
public class BombController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (bombsRemaining > 0 && Input.GetKeyDown(inputKey))
        {
            ...
            Instantiate(bombPrefab).GetComponent<BombBomb>().PlaceBombActive(position);
        }
        ...
        if (bombsRemaining > 0 && Input.GetMouseButtonDown(0) && transform.Find("Weapon").Find("Rocket").gameObject.activeSelf) // 按下鼠標左鍵
        {
            ...
            bombsRemaining--;
            Instantiate(bombPrefab).GetComponent<BombBomb>().ThrowBombActive(transform.position, clickPosition);
