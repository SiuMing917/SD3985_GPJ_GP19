...
namespace Pathfinding {
	...
	public class GraphUpdateSceneEditor : EditorBase {
		...
		public void OnSceneGUI () {
			...
			if (UnityEditor.Tools.current != Tool.View && Event.current.type == EventType.Layout) {
			...
			for (int i = 0; i < points.Count; i++) {
				if (i == selectedPoint && UnityEditor.Tools.current == Tool.Move) {
			...
			if (Event.current.shift && UnityEditor.Tools.current == Tool.Move) {
				HandleUtility.Repaint();
