	...
	public class GridGraphEditor : GraphEditor {
		...
		public override void OnSceneGUI (NavGraph target) {
			...
			if (UnityEditor.Tools.current == Tool.Scale) {
				...
				EditorGUI.BeginChangeCheck();
				center = Handles.PositionHandle(graph.center, UnityEditor.Tools.pivotRotation == PivotRotation.Global ? Quaternion.identity : Quaternion.Euler(graph.rotation));

				if (EditorGUI.EndChangeCheck() && UnityEditor.Tools.viewTool != ViewTool.Orbit) {
					graph.center = center;
				}
